...
from retrieval.retrieval import load_model, load_data, extract_feature, load_query_image, sort_img, \
...
def image_retrieval():
    ...
    if request.method == 'POST':
        if request.form['submit'] == 'retrieval':
            if len(request.files) == 0:
                ...
                if not (f and allowed_file(f.filename)):
                    ...
                    upload_path = os.path.join(basepath, 'static/upload_image', f.filename)
                    f.save(upload_path)

                    # transform image format and name with opencv.
                    # img = cv2.imread(upload_path)
                    # cv2.imwrite(os.path.join(basepath, 'static/upload_image', 'query.jpg'), img)
                    image = 'upload_image/' + f.filename
                    # print(image)
                    start_time = time.time()
                    query_image = load_query_image(os.path.join(basepath, 'static/upload_image', f.filename))
                    ...
                    print(sorted_paths)
                    tmb_images = ['./static/thumb_images/' + os.path.split(sorted_path)[1] for sorted_path in
                                  sorted_paths]
                    return render_template('retrieval.html',
                                           message="Retrieval finished, cost {:3f} seconds.".format(
                                               time.time() - start_time),
                                           query_name=image,
                                           sml1=similarity[0], sml2=similarity[1], sml3=similarity[2],
                                           sml4=similarity[3],
                                           sml5=similarity[4], sml6=similarity[5],
                    ...
                    # return render_template('upload_finish.html', query_name=image, message='Upload successfully!')
